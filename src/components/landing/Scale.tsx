...
// ✅ ТВОИ ЯКОРЯ (месячные), но график будет построен ПО ДНЯМ
// ✅ Старт графика: 24.03.2025
...
/**
 * ✅ Делает дневной ряд между двумя якорями
 * - без random
 * - с детерминированными волнами
 * - НЕ падает (как у каналов)
 */
...
export default function Scale() {
  ...
  const series = useMemo<Point[]>(() => {
    return buildFullSeries(liveCount);
  }, [liveCount]);

  const activity = useMemo(() => {
    if (!data?.updatedAt) return { label: "нет данных", dot: "rgba(255,255,255,0.35)", glow: "none", text: "text-white/60" };
  ...
  return (
    <section id="scale" className="section">
      <div className="max-w-[1180px] mx-auto px-6">
        <div className="rounded-2xl border border-white/10 bg-white/[0.04] backdrop-blur-xl p-8 relative overflow-hidden">
          {/* glow */}
          <div
          ...
          >
            График участников организации WILD
          </motion.h2>

          <p className="mt-3 text-white/70 max-w-2xl">
            График — по дням с <span className="text-white/85">24.03.2025</span>.{" "}
            Конец — <span className="text-white/85">{new Date().toLocaleDateString("ru-RU")}</span> (обновляется).
          </p>
          ...
          <div className="mt-8 rounded-2xl border border-white/10 bg-white/[0.03] p-5">
            ...
            <div className="mt-3 text-xs text-white/45">
              Наведи на график — увидишь точное значение за день.
            </div>
          </div>
        </div>
      </div>
